...
import { map, tap, BehaviorSubject, Observable, catchError } from 'rxjs';
...
export class LicensesService {
  ...
  public deleteLicense(id: string) {
    return this.http.request(Requests['deleteLicense'], null, id).pipe(
      tap(() => this.onDeleteLicense(id)),
      tap(() => this.tools.generateNotification('License deleted', 'primary'))
    );
  }

  /** Removing LICENSE by ID from LicensesArray */
  ...
  public renewLicense(id: string) {
    return this.http.request<License>(Requests['renewLicense'], null, id).pipe(
      map((l) => {
        return { ...l, expires_in: l.expires_in * 1000 };
      }),
      tap((newL) => this.editLicense(newL)),
      tap(() => this.tools.generateNotification('License renewed', 'success'))
    );
  }

  public postLicense(data: Record<any, any>) {
    return this.http.request(Requests['postLicense'], data).pipe(
      map((l) => {
        return { ...l, expires_in: l.expires_in * 1000 };
      }),
  ...
  public putLicense(data: Record<any, any>, id: string) {
    return this.http.request(Requests['putLicense'], data, id).pipe(
      map((l) => {
        return { ...l, expires_in: l.expires_in * 1000 };
      }),
      tap((newL) => this.editLicense(newL)),
  ...
  /** Changing license data in Licenses Arr */
  editLicense(newData: License) {
    this._licenses = this._licenses.map((l) => {
      if (l.id != newData.id) return l;
      return newData;
    });
    this.$licenses.next(this._licenses);
  }
  ...
  public changeReferralScore(licId?: string, points?: number) {
    ...
    let req =
      licId && points
        ? Requests['changeRefPoints']
        : Requests['resetRefPoints'];
  ...
  public updateLicenses() {
    return this.http.request(Requests['getLicenses']).pipe(
      map((licenses) =>
