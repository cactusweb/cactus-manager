...
export class LicenseViewingComponent implements OnInit {
  ...
  constructor(public tools: ToolsService, private lic: LicensesService) {}
  ...
  resetPoints() {
    this.loading = true;
    this.lic
      .changeReferralScore(this.license.id)
      .pipe(
        take(1),
      ...
      .subscribe({
        next: () =>
          this.license.referral ? (this.license.referral.score = 0) : '',
        error: () => {},
      });
  }
  ...
  getExpDate() {
    return (
      this.license.payment.exp_date.slice(0, 2) +
      '/' +
      this.license.payment.exp_date.slice(2, 5)
    );
  }
